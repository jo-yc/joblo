Rails.application.routes.draw do
  ...
  constraints subdomain: // do
    get '/', to: 'blog/home#index', as: :root
    resources :home
    namespace :admin do
    ...
    namespace :blog do
      resources :home, only: :index
      namespace :posts do
        resources :prayer_requests, only: %i[index show]
        resources :drama_team, only: %i[index show]
        resources :korean_ministry, only: %i[index show]
        resources :devotion, only: %i[index show]
        resources :family, only: %i[index show]
        resources :daily_life, only: %i[index show]
        resources :tags, only: %i[create show]
        resources :thoughts, only: %i[index show]
      end
    end
  end
